...
dataset_train = csv_data_class_train(
    path=train_csv_path,
...
dataset_val = csv_data_class_val(
    path=val_csv_path, transform_supervised=transform_val_supervised, transform_moco=transform_val_moco
)
...
for epoch in range(max_number_of_epoch):
    ...
    if top1_val.avg > best_val:
        ...
        torch.save(
            linear_model.state_dict(), "/scratch/mjafarzadeh/linear_classifier_b3_SP_vall_acc_" + str(best_val) + ".pth"
        )
