...
def train_evm(feature_dic, args_evm, gpu):
    with torch.no_grad():
        ...
        for k in range(number_of_classes):
            r = k % NG
            p = mp.Process(
                target=evm_train_process,
                args=([classes_to_process[k]], feature_dic, args_evm, gpu[r], Q, done_event[k]),
            )
            p.start()
            processes.append(p)
            if r == (NG - 1):
...
def validate(evm_model, args_evm, class_partition, feature_dic, data_name, gpu):
    with torch.no_grad():
        ...
        for k in range(NG):
            p = mp.Process(
                target=val_process,
                args=(class_partition[k], feature_dic[k], evm_model, args_evm, gpu[k], Q, done_event[k]),
            )
            p.start()
