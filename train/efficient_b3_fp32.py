...
image_transform_train = transforms.Compose(
    [
        transforms.RandomAffine(
            degrees=45, translate=(0.25, 0.25), scale=(0.5, 4.0), shear=None, resample=PIL.Image.BICUBIC, fillcolor=127
        ),
        transforms.RandomResizedCrop(size=(image_size, image_size), scale=(0.1, 1.0), ratio=(0.5, 1.5)),
...
state_dict_model = torch.load("./trained_efficientnet_b3_fp16_imagenet/trained_efficientnet_b3_fp16_imagenet.pth.tar")
...
for epoch in range(max_number_of_epoch):
    ...
    if top1_val.avg > best_val:
        ...
        torch.save(model.state_dict(), "efficientnet_b3_fp32_imagenet_vall_acc_" + str(best_val) + ".pth.tar")
