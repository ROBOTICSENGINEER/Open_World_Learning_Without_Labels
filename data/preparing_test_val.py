...
df_known = pd.read_csv("imagenet_1000_val.csv", header=None, index_col=None)
...
for u in [0, 5, 10, 25, 50]:
    if u > 0:
        df_index_classes = pd.read_csv(f"tests_val/selected_val_u{u}.txt", header=None, index_col=None)
    else:
        df_index_classes = None

    for test_id in range(5):
        csv_file_output = f"test_val_u{u}_{test_id+1}.csv"
