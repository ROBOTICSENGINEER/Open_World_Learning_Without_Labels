...
with h5py.File(output_h5, "w") as h5:
  for level in levels:
    for test_id in range(Number_of_tests): 
      ...
      score_diff  = np.zeros(number_of_batch)
      score_post  = np.zeros(number_of_batch)
      score_pre = np.zeros(number_of_batch)
      ...
      for index,csv_file in enumerate(charcterization_csv_fils_pre):
        ...
        score_pre[index] = ( correct +  ( b3 * N_UU ) ) /  N_ALL
      ...
      for index,csv_file in enumerate(charcterization_csv_fils_post):
        ...
        if N_UU > 0:
          ...
          b3 = 0 
        score_post[index] = ( correct +  ( b3 * N_UU ) ) /  N_ALL
