...
def EVM_Params(parser):
    ...
    EVM_params_parser.add_argument(
        "--tailsize", nargs="+", type=float, default=[1.0], help="tail size to use\ndefault: %(default)s"
    )
    EVM_params_parser.add_argument(
        "--cover_threshold", nargs="+", type=float, default=[0.7], help="cover threshold to use\ndefault: %(default)s"
    )
    EVM_params_parser.add_argument(
        "--distance_multiplier",
        nargs="+",
    ...
    EVM_params_parser.add_argument(
        ...
        type=int,
        default=200,
        help="Number of classes per chunk, reduce this parameter if facing OOM " "error\ndefault: %(default)s",
    )
    return parser, dict(
...
def set_cover(mr_model, positive_distances, cover_threshold):
    ...
    smallScore = torch.gather(params["smallScoreTensor"][:, 0].to("cpu"), 0, extreme_vectors_indexes)
...
def EVM_Training(pos_classes_to_process, features_all_classes, args, gpu, models=None):
    ...
    for pos_cls_name in pos_classes_to_process:
        ...
        for batch_no, neg_features in enumerate(negative_classes_for_current_class):
            ...
            bottom_k_distances = [
                torch.topk(
                    bottom_k_distances, min(tailsize, bottom_k_distances.shape[1]), dim=1, largest=False, sorted=True
                ).values
            ]
            del distances
        ...
        assert (
            torch.allclose(
                positive_distances[e].type(torch.FloatTensor), torch.zeros(positive_distances.shape[0]), atol=1e-06
            )
            == True
