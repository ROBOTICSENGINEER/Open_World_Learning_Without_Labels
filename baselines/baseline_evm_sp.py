...
with torch.no_grad():
    ...
    if torch.min(data_train_evm[:, 0]) == 0:
        assert torch.max(data_train_evm[:, 0]) == number_of_known_classes - 1
        data_train_evm[:, 0] = data_train_evm[:, 0] + 1
    features_dict_train = OrderedDict()
    assert torch.min(data_train_evm[:, 0]) == 1
    assert torch.max(data_train_evm[:, 0]) == number_of_known_classes
