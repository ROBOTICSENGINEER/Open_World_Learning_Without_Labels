...
from Centroid_Based_Concept_Learning import *
...
parser.add_argument('--ood', required=True, choices=['softmax', 'energy', 'evm'],
                                                  help='OOD type: SoftMax, Energy, EVM.')
...
CBCL = Centroid_Based_Concept_Learning(top_k=1, threshold_distance=15)
...
with torch.no_grad():
  ...
  for i, (image_names, x_supervised, x_moco) in enumerate(test_loader, 0):
    ...
    probability_tensor = torch.zeros(batch_size, n)
    ...
    if torch.sum(predicted_unknown) > 0:
      if number_of_discovered_classes > 0:
        ...
        p_given_unknown = CBCL.predict(FV_predicted_unknown).cpu()
    ...
    if len(residual_dict) > 0:
      ...
      assert len(class_to_process) > 0
      CBCL.fit(class_to_process, features_dict_incremental)
