...
NNO = Nearest_Non_Outlier(tau=2.0, rank = 4, learning_rate = 0.001, batch_size = 500, 
                           num_workers = n_cpu,  max_number_of_epoch = 200, gpu = 0)
...
for i, (image_names, x_supervised, x_moco) in enumerate(test_loader, 0):
  with torch.no_grad():
    ...
    if torch.sum(predicted_unknown) > 0:
      if number_of_discovered_classes > 0:
        ...
        p_given_unknown = NNO.predict(FV_predicted_unknown).cpu()
  ...
  if len(residual_dict) > 0:
    ...
    if (  ((len(class_to_process) > 0) and (number_of_discovered_classes > 0))
      ...
      NNO.fit(class_to_process, features_dict_incremental)
    else:
      nu = 0
