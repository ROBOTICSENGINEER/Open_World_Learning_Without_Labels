...
SCAIL = Scaling_Incremental_Learning(feature_size = feature_size, buffer_size = 250, 
                                   softmax_threshold = 0.5, learning_rate = 0.001,
                                   batch_size = 4096, num_workers = n_cpu, 
                                   max_number_of_epoch = 200, gpu = 0)
  ...
  with torch.no_grad():
    ...
    if torch.sum(predicted_unknown) > 0:
      if number_of_discovered_classes > 0:
        ...
        p_given_unknown = SCAIL.predict(FV_predicted_unknown).cpu()
