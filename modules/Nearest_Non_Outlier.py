...
class Nearest_Non_Outlier(object):
  def __init__(self, tau=1.0, rank = 2, gpu = None, clipping_value = 256, max_number_of_epoch = 200,
               learning_rate = 0.001, batch_size = 8, num_workers = 1):
  ...
  def fit(self, class_to_process, features_dict_new):
    ...
    train_loader = DataLoader(dataset=dataset_train, batch_size=self.batch_size, 
    ...
    self.NCMML = Nearest_classifier_Mean_Metric_learning(
                                centers =  self.centers, rank = self.rank, W = self.old_w)
