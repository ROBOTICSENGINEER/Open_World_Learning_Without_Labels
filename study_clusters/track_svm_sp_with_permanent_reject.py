...
with torch.no_grad():
  ...
  for i, (image_names, x_supervised, x_moco) in enumerate(test_loader, 0):
    ...
    if len(residual_dict) >= min_number_point_to_start_clustering:
        ...
        for cluster_number in range(number_of_clusters):  # number of clusters after clustering.
          ...
          if len(index) >= min_number_point_to_create_evm:
            ...
            if cluster_quality_checker.run(feature_this_cluster) == 1:
              ...
              if args_owl.n == 0:
                index_file = str(i+1).zfill(2)
                output_path = config["result_dir"] + args_owl.output_name + f'_new_clusters_{index_file}.txt'
              ...
              # cluster rejected by svm
              to_be_delete = to_be_delete + index
              for jjjj in index:
                rejected_dict[image_names_residual[jjjj]] = FVs_residual[jjjj]
      ...
      print(f"len(residual_dict) = {len(residual_dict)}")
      print(f"len(rejected_dict) = {len(rejected_dict)}")
  ...
  if args_owl.n == 0:
    ...
    storage_dict_final = dict()
    N = 1 + number_of_known_classes + number_of_discovered_classes
    
    image_names_rejected, FV_rejected = zip(*rejected_dict.items())
    FV_rejected =  torch.from_numpy(np.vstack(list(FV_rejected)))
    print("FV_rejected.shape = ", FV_rejected.shape)

    image_names_rest = []
    FV_rest = []
    for im_1, fv_1 in zip(image_list, FV_track):
      if not im_1 in image_names_rejected:
        image_names_rest.append(im_1)
        FV_rest.append(fv_1)
    FV_rest =  torch.vstack(FV_rest)
    print("FV_rest.shape = ", FV_rest.shape)
    

    
    feature_dict_rejected = OrderedDict()
    feature_dict_rejected[0] = FV_rejected.double()
    Pr_iterator_rejected = EVM_Inference([0], feature_dict_rejected, args_evm, 0, evm_model)
    for j,pr_rejected in enumerate(Pr_iterator_rejected):
      prob_rejected  = pr_rejected[1][1]#.cuda()
      assert j == 0
    del Pr_iterator_rejected, pr_rejected
    gc.collect()
    torch.cuda.empty_cache()
    torch.cuda.ipc_collect()
    probability_tensor_rejected = torch.zeros(prob_rejected.shape[0], N)
    probability_tensor_rejected[:,1:] = prob_rejected
    P_max_all_rejected , _  = torch.max(prob_rejected, axis=1)
    pu_rejected = 1 -  P_max_all_rejected
    probability_tensor_rejected[:,0] = pu_rejected
    norm_rejected = torch.norm(probability_tensor_rejected, p=1, dim=1)
    normalized_tensor_rejected = probability_tensor_rejected/norm_rejected[:,None]
    P_max_rejected , i_max_rejected  = torch.max(normalized_tensor_rejected, axis=1)
    
    for clas_number in range(N):
      is_in_class = (i_max_rejected == clas_number)
      if torch.sum(is_in_class) > 0:
        storage_dict_final[f"{clas_number}_rejected"] = []
        for jj, flag in enumerate(is_in_class):
          if flag:
            storage_dict_final[f"{clas_number}_rejected"].append(image_names_rejected[jj])



    feature_dict_rest = OrderedDict()
    feature_dict_rest[0] = FV_rest.double()
    Pr_iterator_rest = EVM_Inference([0], feature_dict_rest, args_evm, 0, evm_model)
    for j,pr_rest in enumerate(Pr_iterator_rest):
      prob_rest  = pr_rest[1][1]#.cuda()
      assert j == 0
    del Pr_iterator_rest, pr_rest
    gc.collect()
    torch.cuda.empty_cache()
    torch.cuda.ipc_collect()
    probability_tensor_rest = torch.zeros(prob_rest.shape[0], N)
    probability_tensor_rest[:,1:] = prob_rest
    P_max_all_rest , _  = torch.max(prob_rest, axis=1)
    pu_rest = 1 -  P_max_all_rest
    probability_tensor_rest[:,0] = pu_rest
    norm_rest = torch.norm(probability_tensor_rest, p=1, dim=1)
    normalized_tensor_rest = probability_tensor_rest/norm_rest[:,None]
    P_max_rest , i_max_rest  = torch.max(normalized_tensor_rest, axis=1)
    
    for clas_number in range(N):
      is_in_class = (i_max_rest == clas_number)
      if torch.sum(is_in_class) > 0:
        storage_dict_final[f"{clas_number}_rest"] = []
        for jj, flag in enumerate(is_in_class):
          if flag:
            storage_dict_final[f"{clas_number}_rest"].append(image_names_rest[jj])
            
    torch.save(storage_dict_final , config["result_dir"] + args_owl.output_name + f'_final_stage.pth')
  t9 = time.time()
