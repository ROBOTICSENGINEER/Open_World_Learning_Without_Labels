...
class csv_data_class(Dataset):
  ...
  def __init__(self, path, transform_moco):
    with open(path) as f:
...
dataset_test = csv_data_class(path = test_csv_path, transform_moco = image_transform_moco)
...
with torch.no_grad():
  ...
  FV_track = torch.zeros(5000, 1536).cuda()
  ...
  image_list = [] 
  for i, (image_names, x_moco) in enumerate(test_loader, 0):
    t5 = time.time()
    print("\nbatch ", i+1)
    x_moco = x_moco.cuda()
