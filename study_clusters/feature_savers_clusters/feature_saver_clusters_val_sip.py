...
t0 = time.time()
_name_feature_extractor_ = "SIP"
folder = "/scratch/mjafarzadeh/result_val_sip/"
output_folder = f"/scratch/mjafarzadeh/result_val_sip/"
output_name = f"feature_cluster_val_sip.pth"

batch_size = 512

number_of_known_classes = 1000
cnn_path_supervised = "/scratch/mjafarzadeh/trained_efficientnet_b3_fp16_imagenet.pth"
cnn_path_moco_imagenet = "/scratch/mjafarzadeh/moco_imagenet_0199.pth"
...
model_moco_imagenet = net_model_from_lib(num_classes = number_of_known_classes)  # timm library
...
for k in list(state_dict.keys()):
  ...
  del state_dict[k]
msg = model_moco_imagenet.load_state_dict(state_dict, strict=False)
assert set(msg.missing_keys) == {"classifier.weight", "classifier.bias"}
for parameter in model_moco_imagenet.parameters():
  parameter.requires_grad = False
model_moco_imagenet.cuda()
model_moco_imagenet.eval()
...
with torch.no_grad():
  ...
  for u in [5, 10,25,50]:
    for test_id in range(1,6):
      print(f"unknown {u} , test_id {test_id}", )
      
      file_list = []
      for file in os.listdir(folder):
        if f'track_val_sip_u{u}_{test_id}_new_clusters_' in file:
      ...
      for cluster in range(1 , 1 + N_cluster):
        ...
        for batch_id, (x_supervised, x_moco) in enumerate(data_loader, 0):
          ...
          FV_moco_imagenet, _ = model_moco_imagenet(x_moco)
          FV_moco_places, _ = model_moco_places(x_moco)
          FV = torch.cat((F_supervised, FV_moco_imagenet, FV_moco_places), 1)
          FV = FV.cpu()
