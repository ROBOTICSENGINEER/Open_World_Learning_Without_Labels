...
t0 = time.time()
folder = "/scratch/mjafarzadeh/result_val/"
output_folder = f"/scratch/mjafarzadeh/"
output_name = f"feature_cluster_val.pth"
...
with torch.no_grad():
  ...
  for level in [10,25,50,100]:
    for test_id in range(1,6):
      print(f"percentage {level} , test_id = {test_id}", )
      
      
      file_list = []
      for file in os.listdir(folder):
        if f'track_val_u{level}_{test_id}_new_clusters_' in file:
