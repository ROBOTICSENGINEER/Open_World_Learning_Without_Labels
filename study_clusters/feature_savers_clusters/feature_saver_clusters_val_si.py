...
t0 = time.time()
_name_feature_extractor_ = "SI"
folder = "/scratch/mjafarzadeh/result_val_si/"
output_folder = f"/scratch/mjafarzadeh/result_val_si/"
output_name = f"feature_cluster_val_si.pth"
  ...
  for u in [5, 10,25,50]:
    for test_id in range(1,6):
      ...
      for file in os.listdir(folder):
        if f'track_val_si_u{u}_{test_id}_new_clusters_' in file:
      ...
      for cluster in range(1 , 1 + N_cluster):
        ...
        for batch_id, (x_supervised, x_moco) in enumerate(data_loader, 0):
          ...
          FV = torch.cat((F_supervised, FV_moco_imagenet), 1)
          FV = FV.cpu()
